...
namespace EspinosaHomeAct
{
    public partial class MainPage : ContentPage
    {
        ...
        private async void OnAddUserClicked(object sender, EventArgs e)
        {
            ...
            try
            {
                var newUser = new Model.User { Name = NameEntry.Text.Trim() , PhoneNumber = NumberEntry.Text.Trim()};
        ...
        private async void OnEditUserClicked(object sender, EventArgs e)
        {
            if (sender is Button button && button.CommandParameter is User userToEdit)
            {
                ...
                string newPhone = await DisplayPromptAsync("Edit User", "Enter new phone number:", initialValue: userToEdit.PhoneNumber, keyboard: Keyboard.Telephone);
                if (string.IsNullOrWhiteSpace(newPhone))
                {
                    await DisplayAlertAsync("Error", "Phone number cannot be empty.", "OK");
                    return;
                }
                try
                {
                    userToEdit.Name = newName.Trim();
                    userToEdit.PhoneNumber = newPhone.Trim();
